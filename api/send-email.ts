...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!smtpHost || !smtpPort || !smtpUser || !smtpPass || !mailFrom) {
      ...
      return res.status(500).json({ success: false, message: "Email service is not configured. Please set SMTP_HOST, SMTP_PORT, SMTP_USER, SMTP_PASS, and MAIL_FROM." });
    }
    ...
    console.log("Vercel: Contact Form Submission:", { timestamp: new Date().toISOString(), fromName: name, fromEmail: email, toEmail: to });
